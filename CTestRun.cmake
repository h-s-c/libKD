...
if(CTEST_COVERAGE_COMMAND)
    ...
    if(CC_NAME STREQUAL "msvc")
        ...
        while(OCC_ABORT GREATER -1)
            ...
            exec_program(OpenCppCoverage ${CTEST_BINARY_DIRECTORY}/Debug ARGS --sources ${CTEST_SOURCE_DIRECTORY}\\source --modules *libKD* --cover_children --export_type=cobertura -- ${OCC_TEST}.exe)
        ...
        endwhile()
        exec_program(python ${CTEST_SOURCE_DIRECTORY} ARGS coverage_merge_script.py -p "${CTEST_BINARY_DIRECTORY}\\Debug" -o "${CTEST_BINARY_DIRECTORY}\\coverage.xml")
        file(READ ${CTEST_BINARY_DIRECTORY}/coverage.xml OCC_COVXML)
        string(REGEX REPLACE "<\\?xml[^\r\n\t\\\\]+" "<?xml version=\"1.0\"?>\\n<!--DOCTYPE coverage SYSTEM \"http://cobertura.sourceforge.net/xml/coverage-03.dtd\"-->" OCC_COVXML ${OCC_COVXML})
        string(REGEX REPLACE "<lines>" "<methods/>\n          <lines>" OCC_COVXML ${OCC_COVXML})
        ...
        set(OCC_ABORT 0)
        while(OCC_ABORT EQUAL 0)
            string(REGEX MATCH " name=[^ ]+\\.c." OCC_EXT1 ${OCC_COVXML})
            if(OCC_EXT1 STREQUAL "")
                set(OCC_ABORT 1)
            else()
                string(REGEX REPLACE "\\.c" "" OCC_EXT2 ${OCC_EXT1})
                string(REGEX REPLACE ${OCC_EXT1} ${OCC_EXT2} OCC_COVXML ${OCC_COVXML})
            endif()
        endwhile()
        string(REGEX REPLACE "<source>[^ ]+</source>" "<source>source<source>" OCC_COVXML ${OCC_COVXML})
