...
elseif(CC_NAME STREQUAL "msvc" AND NOT CC_ARCH STREQUAL "arm")
    #Dummy command
    find_program(CTEST_COVERAGE_COMMAND NAMES cmd)   
endif()
if(CTEST_COVERAGE_COMMAND)
    ...
    if(CC_NAME STREQUAL "msvc")
        ...
        while(OCC_ABORT LESS 1)
            ...
            if(OCC_LASTTEST STREQUAL "")
                exec_program(OpenCppCoverage ${CTEST_BINARY_DIRECTORY}/Debug ARGS --sources ${CTEST_SOURCE_DIRECTORY}\\source --modules *libKD* --cover_children --export_type=binary -- ${OCC_TEST}.exe)
            else()
                exec_program(OpenCppCoverage ${CTEST_BINARY_DIRECTORY}/Debug ARGS --sources ${CTEST_SOURCE_DIRECTORY}\\source --modules *libKD* --cover_children --export_type=binary --input_coverage=${OCC_LASTTEST}.cov -- ${OCC_TEST}.exe)
            endif()
            set(OCC_LASTTEST ${OCC_TEST})
            string(FIND ${OCC_TESTS} "#" OCC_ABORT_CHECK)
            if(OCC_ABORT_CHECK LESS 0)
                exec_program(OpenCppCoverage ${CTEST_BINARY_DIRECTORY}/Debug ARGS --sources ${CTEST_SOURCE_DIRECTORY}\\source --modules *libKD* --cover_children --export_type=cobertura --input_coverage=${OCC_LASTTEST}.cov -- ${OCC_TEST}.exe)
                file(RENAME ${CTEST_BINARY_DIRECTORY}/Debug/${OCC_TEST}Coverage.xml ${CTEST_BINARY_DIRECTORY}/coverage.xml)
                file(READ ${CTEST_BINARY_DIRECTORY}/coverage.xml OCC_COVXML)
                string(REGEX REPLACE "name=[^ ]+\\build\\\\Debug." "filename=\"build/Debug/" OCC_COVXML ${OCC_COVXML})
                string(REGEX REPLACE "filename=[^ ]+\\source." "filename=\"source/" OCC_COVXML ${OCC_COVXML})
                file(WRITE ${CTEST_BINARY_DIRECTORY}/coverage.xml ${OCC_COVXML})
                set(OCC_ABORT 1)
            endif()
        endwhile()
    endif()
