...
static const KDfloat64KHR cst4pio2 = 4*KD_PI_2_KHR;   /* 0x401921FB, 0x54442D18 */
...
static inline KDfloat32 __kernel_cosdf(KDfloat64KHR x)
{
    KDfloat64KHR r, w, z;
...
static KDfloat64KHR __scalbn (KDfloat64KHR x, KDint n)
{
    ...
    if (k==0) {             /* 0 or subnormal x */
        ...
        k = k+n;
        if (k >  0x7fe) return huge*__kdCopysign(huge,x); /* overflow  */
...
static KDfloat32 __kdScalbnf (KDfloat32 x, KDint n)
{
    ...
    if (k==0xff) return x+x;        /* NaN or Inf */
        k = k+n;
        if (k >  0xfe) return huge*__kdCopysignf(huge,x); /* overflow  */
    if (k > 0)              /* normal result */
        {SET_FLOAT_WORD(x,(ix&0x807fffff)|(k<<23)); return x;}
    if (k <= -25) {
        if (n > 50000)  /* in case integer overflow in n+k */
        return huge*__kdCopysignf(huge,x);  /*overflow*/
        else return tiny*__kdCopysignf(tiny,x); /*underflow*/
    }
...
static KDint __kernel_rem_pio2(KDfloat64KHR *x, KDfloat64KHR *y, KDint e0, KDint nx, KDint prec)
{
    ...
    j = jv-jx; m = jx+jk;
    for(i=0;i<=m;i++,j++) f[i] = (j<0)? zero : (KDfloat64KHR) ipio2[j];
...
static inline KDint __ieee754_rem_pio2f(KDfloat32 x, KDfloat64KHR *y)
{
    KDfloat64KHR w,r,fn;
...
KD_API KDfloat32 KD_APIENTRY kdAcosf(KDfloat32 x)
{
    ...
    if(ix>=0x3f800000) {        /* |x| >= 1 */
        ...
        else return pi+(KDfloat32)2.0*pio2_lo;  /* acos(-1)= pi */
        }
...
KD_API KDfloat32 KD_APIENTRY kdAtanf(KDfloat32 x)
{
    ...
    if(ix>=0x4c800000) {    /* if |x| >= 2**26 */
        if(ix>0x7f800000)
        return x+x;     /* NaN */
        if(hx>0) return  atanhi[3]+*(volatile KDfloat32 *)&atanlo[3];
        else     return -atanhi[3]-*(volatile KDfloat32 *)&atanlo[3];
    ...
    if (ix < 0x3f980000) {      /* |x| < 1.1875 */
        ...
        id = 0; x = ((KDfloat32)2.0*x-one)/((KDfloat32)2.0+x);
        } else {            /* 11/16<=|x|< 19/16 */
        id = 1; x  = (x-one)/(x+one);
        }
    } else {
        if (ix < 0x401c0000) {  /* |x| < 2.4375 */
        id = 2; x  = (x-(KDfloat32)1.5)/(one+(KDfloat32)1.5*x);
        } else {            /* 2.4375 <= |x| < 2**26 */
        id = 3; x  = -(KDfloat32)1.0/x;
        }
    }}
...
KD_API KDfloat32 KD_APIENTRY kdAtan2f(KDfloat32 y, KDfloat32 x)
{
    ...
    if((ix>0x7f800000)||
       ...
       return x+y;
    if(hx==0x3f800000) return kdAtanf(y);   /* x=1.0 */
...
KD_API KDfloat32 KD_APIENTRY kdCosf(KDfloat32 x)
{
    ...
    if(ix<=0x407b53d1) {        /* |x| ~<= 5*pi/4 */
        ...
        return -__kernel_cosdf(x + (hx > 0 ? -cs2pio2 : cs2pio2));
        else {
        if(hx>0)
            return __kernel_sindf(cs1pio2 - x);
        else
            return __kernel_sindf(x + cs1pio2);
        }
    }
    if(ix<=0x40e231d5) {        /* |x| ~<= 9*pi/4 */
        if(ix>0x40afeddf)       /* |x|  ~> 7*pi/4 */
        return __kernel_cosdf(x + (hx > 0 ? -cst4pio2 : cst4pio2));
        else {
        if(hx>0)
            return __kernel_sindf(x - cs3pio2);
        else
            return __kernel_sindf(-cs3pio2 - x);
        }
    }
...
KD_API KDfloat32 KD_APIENTRY kdSinf(KDfloat32 x)
{
    ...
    if(ix <= 0x3f490fda) {      /* |x| ~<= pi/4 */
        if(ix<0x39800000)       /* |x| < 2**-12 */
        if(((KDint)x)==0) return x;   /* x with inexact if x != 0 */
        return __kernel_sindf(x);
    }
    if(ix<=0x407b53d1) {        /* |x| ~<= 5*pi/4 */
        if(ix<=0x4016cbe3) {    /* |x| ~<= 3pi/4 */
        if(hx>0)
            return __kernel_cosdf(x - cs1pio2);
        else
            return -__kernel_cosdf(x + cs1pio2);
        } else
        return __kernel_sindf((hx > 0 ? cs2pio2 : -cs2pio2) - x);
    }
    if(ix<=0x40e231d5) {        /* |x| ~<= 9*pi/4 */
        if(ix<=0x40afeddf) {    /* |x| ~<= 7*pi/4 */
        if(hx>0)
            return -__kernel_cosdf(x - cs3pio2);
        else
            return __kernel_cosdf(x + cs3pio2);
        } else
        return __kernel_sindf(x + (hx > 0 ? -cst4pio2 : cst4pio2));
    }
...
KD_API KDfloat32 KD_APIENTRY kdTanf(KDfloat32 x)
{
    ...
    if(ix<=0x407b53d1) {        /* |x| ~<= 5*pi/4 */
        if(ix<=0x4016cbe3)      /* |x| ~<= 3pi/4 */
        return __kernel_tandf(x + (hx>0 ? -cs1pio2 : cs1pio2), -1);
        else
        return __kernel_tandf(x + (hx>0 ? -cs2pio2 : cs2pio2), 1);
    }
    if(ix<=0x40e231d5) {        /* |x| ~<= 9*pi/4 */
        if(ix<=0x40afeddf)      /* |x| ~<= 7*pi/4 */
        return __kernel_tandf(x + (hx>0 ? -cs3pio2 : cs3pio2), -1);
        else
        return __kernel_tandf(x + (hx>0 ? -cst4pio2 : cst4pio2), 1);
    }
...
{
    KDfloat32 y,hi=0.0,lo=0.0,c,t,twopk;
    KDint32 k=0,xsb;
    KDuint32 hx;
    GET_FLOAT_WORD(hx,x);
...
KD_API KDfloat32 KD_APIENTRY kdLogf(KDfloat32 x)
{
    ...
    if((0x007fffff&(0x8000+ix))<0xc000) {   /* -2**-9 <= f < 2**-9 */
        ...
        R = f*f*((KDfloat32)0.5-(KDfloat32)0.33333333333333333*f);
...
KD_API KDfloat32 KD_APIENTRY kdPowf(KDfloat32 x, KDfloat32 y)
{
    ...
    /* now |1-x| is tiny <= 2**-20, suffice to compute
       log(x) by x-x^2/2+x^3/3-x^4/4 */
        ...
        w = (t*t)*((KDfloat32)0.5-t*((KDfloat32)0.333333333333-t*(KDfloat32)0.25));
    ...
    /* compute log(ax) */
        ...
        r += s_l*(s_h+s);
        s2  = s_h*s_h;
        t_h = (KDfloat32)3.0+s2+r;
        GET_FLOAT_WORD(is,t_h);
        SET_FLOAT_WORD(t_h,is&0xfffff000);
        t_l = r-((t_h-(KDfloat32)3.0)-s2);
    /* u+v = s*(1+...) */
        u = s_h*t_h;
        v = s_l*t_h+t_l*s;
    /* 2/(3log2)*(s+...) */
    ...
    else if ((KDuint32)j==0xc3160000){            /* z == -150 */
    ...
    GET_FLOAT_WORD(j,z);
    j += (n<<23);
    if((j>>23)<=0) z = __kdScalbnf(z,n);    /* subnormal output */
...
static KDfloat64KHR __kdFloorKHR_Generic(KDfloat64KHR x)
{
    ...
    if(j0<20) {
        ...
        if(i0<0) {
            ...
            j = i1+(1<<(52-j0));
            if((KDint32)j<i1) i0 +=1 ;   /* got a carry */
            i1=j;
            }
        }
        i1 &= (~i);
