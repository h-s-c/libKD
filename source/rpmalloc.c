...
#endif
#if __has_warning("-Wbad-function-cast")
...
typedef KDAtomicPtrVEN atomicptr_t;
...
static FORCEINLINE int     atomic_cas_ptr(atomicptr_t* dst, void* val, void* ref) { return kdAtomicPtrCompareExchangeVEN(dst, val, ref); }
...
static FORCEINLINE int     atomic_cas32_acquire(atomic32_t* dst, int32_t val, int32_t ref) { return kdAtomicIntCompareExchangeVEN(dst, val, ref); }
...
#if defined(__clang__)
static FORCEINLINE void*   atomic_exchange_ptr_acquire(atomicptr_t* dst, void* val) { return __sync_swap(dst, val); }
#else
...
/* crt replacment for tls */
#if defined(KD_FREESTANDING) && defined(_MSC_VER)
...
_rpmalloc_global_cache_insert_spans(span_t** span, size_t span_count, size_t count) {
	...
	kdMemcpy(cache->span + cache->count, span, sizeof(span_t*) * insert_count);
...
_rpmalloc_global_cache_extract_spans(span_t** span, size_t span_count, size_t count) {
	...
	kdMemcpy(span + extract_count, cache->span + (cache->count - want), sizeof(span_t*) * want);
...
rpmalloc_initialize_config(const rpmalloc_config_t* config) {
		...
		if (_memory_config.enable_huge_pages) {
			...
			if (meminfo) {
				...
				while (!huge_page_size && fgets(line, sizeof(line) - 1, meminfo)) {
					...
					if (kdStrstrVEN(line, "Hugepagesize:"))
	...
	_huge_pages_peak = 0;
#endif
	kdMemset(_memory_heaps, 0, sizeof(_memory_heaps));
	atomic_store32_release(&_memory_global_lock, 0);
...
rpmalloc_heap_free_all(rpmalloc_heap_t* heap) {
	...
	kdMemset(heap->size_class, 0, sizeof(heap->size_class));
	kdMemset(heap->full_span, 0, sizeof(heap->full_span));
